...
export default function ProductAction({ id, title, sprice, mrp, categoryLabel, url, stock, admin }) {
  ...
  const { inCart, toggleCart, shareProduct } = useCart({ id, title, sprice, mrp, categoryLabel, url });
  // const { user } = useAuth();
  // const admin = user.access > 1;


  const iconSx = {
    bgcolor: palette.grey[100], height: 45, width: 45,
      ...
      >
        <Box className='w-50 d-flex'>
          ...
          >
            ...
            {
              !admin && inCart && <IconButton
                sx={{ width: 45, bgcolor: palette.primary.main, color: "#fff" }}
                color='inherit'
                component={Link}
                to={PATHS.app.cart}
              >
