...
export default function SearchPage({ admin }) {
  ...
  const {
    ...
    loading,
    onSearchChange
  } = useProducts({ fetchAllProducts: false });
  ...
  return (
    <Page title="Search" color='paper'>
      ...
      <Container maxWidth='xl' sx={{ pt: 2 }}>
        ...
        {!loading ? (<>
          {
            searchedProducts.length === 0 && <NoData
              width={180}
              sx={{ pt: '100px!important' }}
              text="Products not found"
              description={Object.keys(filters).length > 0 ? "Try removing filters" : "Try searching with different keyword"}
            >
              {
                Object.keys(filters).length > 0 && <Button color='error' onClick={onRemoveFilters}>Remove filters</Button>
              }
          ...
          {
            !lastPage && searchedProducts.length > 0 && <Stack direction='row' className="w-100 py-4 jc-c">
              <Button onClick={search} variant='soft'>Load more</Button>
            </Stack>
          }
        </>) :
...
function SearchBox({
  ...
  return (<Stack
    className='p-s t-0 zi-10'
    sx={{ backdropFilter: 'blur(8px)' }}
  >
    <Stack
      direction='row'
      sx={{ bgcolor: 'background.paper' }}
      className='p-05 ai-c w-100 jc-sb'
      spacing={1}
    >
      <IconButton
        size='large'
