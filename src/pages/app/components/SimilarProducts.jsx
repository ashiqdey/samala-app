...
export default function SimilarProducts({ id, admin }) {
  ...
  return (
    <>
      <Stack direction="row" className="jc-sb ai-c py-2">
        <Typography variant="h3">You might like{admin ? 'admin' : 0}</Typography>
        <GridSwitch />
      </Stack>
      <Grid container spacing={2}>
        ...
        {similars.map((e) => (
          <Grid
            ...
            // lg={grid ? 3 : 12}
          >
            {!admin ? <ProductCard {...e} grid={grid} /> : <ProductCardAdmin {...e} action="admin" grid={grid} />}
          </Grid>
        ))}
      </Grid>
    </>
