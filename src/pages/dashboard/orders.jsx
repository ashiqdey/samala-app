    ...
    <Page title="Orders" color="paper">
      <Stack direction="row" className="p-s t-0 zi-100 ai-c" sx={{ bgcolor: 'grey.100' }}>
        <Tabs value={tab} onChange={handleChange} className="w-100">
          {TABS.map((e) => (
            <Tab key={e.id} {...e} sx={{ px: 1, mr: '0!important', width: 'calc(50% - 26px)' }} />
          ))}
          <Tab label={<></>} id="sync-tab" sx={{ mr: '0!important', width: 48, height: 48 }} />
        </Tabs>
      </Stack>

      <Container maxWidth="xl">
        {tab === 0 ? (
          <OrdersSection orders={orders} nextId={nextId} type="fresh" />
        ) : (
          <OrdersSection orders={porders} nextId={pnextId} type="processed" />
        )}
      </Container>
    </Page>
  );
  ...
  return (
    <>
      ...
      <Stack spacing={2} className="py-2">
        {orders.map((e) => (
          <OrderCardAdmin key={e.id} {...e} type={type} />
        ))}
      </Stack>
      ...
      <Stack direction="row" className="py-5 jc-c">
        ...
        {!loading && nextId > -1 && (
          <Button variant="soft" onClick={() => fetchOrders()}>
            Load more
          </Button>
        )}
      </Stack>
