  ...
  return (
    <Box>
      <Grid container spacing={2} className="product">
        {images.map((image) => (
          <Grid key={image} item xs={6}>
            <Box className="w-100 p-r">
              <ButtonBase
                className="pimg-wrap pimg-holder p-r w-100 bg-white br-15"
        ...
        {images.length < 5 && (
          <Grid item xs={images.length === 0 ? 12 : 6}>
            <ButtonBase
              className="br-15 w-100 d-block"
              sx={{
                color: 'primary.main',
            ...
            >
              <div className="pimg-holder p-r d-flex ai-c jc-c ta-c">
                <div className="p-a w-100 t-50 l-50 tt--50">
                  <SvgIconStyle sx={{ width: 70, height: 70 }} src="/assets/samala/icons/duo-image.svg" />
      ...
      <UploadImage open={uploadDialog} onClose={() => setUploadDialog(false)} onUpload={onImageUpload} type="product" />
      <ReorderImages
        images={images}
        open={reorderImages}
