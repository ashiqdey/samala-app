...
export default function Cart() {
  ...
  const summary = useMemo(() => {
    ...
    const [mrp, sprice] = carts.reduce((a, e) => [a[0] + e.mrp * e.cartQty, a[1] + e.sprice * e.cartQty], [0, 0]);
  ...
  const onPlaceOrder = async () => {
    if (summary.total < config.MIN_ORDER_VALUE) {
      enqueueSnackbar(`Minimul order value is ${config.MIN_ORDER_VALUE}`, { variant: 'error' });
      return;
    }
    ...
    return (
      <Grid container spacing={2}>
        <Stack className="ai-c jc-c w-100 dark-brightness-07" sx={{ height: 'calc(100vh - 180px)', ml: 2 }}>
          ...
          <Typography variant="font4">Your cart is empty</Typography>
        </Stack>
      </Grid>
    );
  }
  ...
  return (
    <Stack>
      <Stack spacing={2}>
        {carts.map((e) => (
          <CartProductCard key={e.id} {...e} />
        ))}
      </Stack>

      <Stack direction="row" className="pt-3 pb-6 jc-c w-100">
        {loading && <CircularProgress />}
        {!loading && cartNextId !== -1 && (
          <Button onClick={fetchCart} variant="text">
            Load more
          </Button>
        )}
      </Stack>
      ...
      {!loading && carts.length && (
        <>
          ...
          {/* <Alert className="mt-2" severity="warning">
            Delivery charge might change during order approval.
          </Alert> */}

          <Address data={data} setData={setData} />

          {summary.total < config.MIN_ORDER_VALUE && (
            <>
              <Alert className="mt-4" severity="error">
                <strong>Minimum order value is ₹{config.MIN_ORDER_VALUE}</strong>
                <p>Add additional item worth of ₹{fCurrency(config.MIN_ORDER_VALUE - summary.total)} to place order</p>
              </Alert>
            </>
          )}
          ...
          <Stack direction="row" className="jc-c w-100">
            {ploading ? (
              ...
              <Button
                disabled={summary.total < config.MIN_ORDER_VALUE}
                variant="contained"
    ...
    <>
      ...
      <Paper className="p-2 mt-2">
        <Stack direction="row" className="jc-sb ai-c">
          ...
          <Button
            ...
            startIcon={
              <Iconify
                icon={data.phone.length < 10 || data.address.length === 0 ? 'mdi:plus' : 'mdi:pencil'}
                width={16}
                height={16}
              />
            }
        ...
        {data.phone.length > 0 && data.address.length > 0 && (
          <Box className="mt-2">
      ...
      <Box className="mt-2">
        ...
        <Typography variant="subtitle2" color="text.secondary">
          There are certain medicine that requires doctor's prescription, If required so, it will be checked during
          delivery. Make sure to keep it handy while receiving the delivery.
        </Typography>
      </Box>

      <AddressDialog open={addressModal} onClose={() => setAddressModal(false)} onConfirm={setData} data={data} />
    </>
  );
}
Address.propTypes = {
...
function Summary({ summary }) {
  return (
    <Paper className="p-2">
      ...
      <Alert className="mt-1 mb-2" severity="warning">
        Delivery charge might change during order approval.
      </Alert>
